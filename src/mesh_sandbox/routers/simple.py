...
@router.get("/reset", status_code=status.HTTP_200_OK, include_in_schema=False, response_model_exclude_none=True)
async def reset(
    clear_disk: str = Query(
        default=None,
        title="Clear disk",
        description="whether to clear the filesystem store (if STORE_MODE=file)",
        example="true",
    ),
    handler: ResetHandler = Depends(ResetHandler),
):
    await handler.reset(clear_disk)
